...
int i2c_write(unsigned char slave_addr, unsigned char reg_addr, unsigned char length, unsigned char *data){
	if(length==1){
		...
		/*char buff[30];
		sprintf(buff,"\nWR1->ADDR: %02x, REG: %02x, DATA: %d",slave_addr,reg_addr,*data);
		uart_sendText(buff);
		sprintf(buff," <<%d>> ",ret);
		uart_sendText(buff);*/
		ret = 0;
		...
		for (unsigned char i=0;i<length;i++){
			ret = i2c_RegisterSet(I2C0, slave_addr*2, reg_addr+i, *(data+i));
		}*/
		int ret = i2c_Register_Write_Block (I2C0, slave_addr*2, reg_addr, length, data);
		/*char buff[30];
		sprintf(buff,"\nWR->ADDR: %02x, REG: %02x, LEN: %d DATA: ",slave_addr,reg_addr,length);
		uart_sendText(buff);
...
int i2c_read(unsigned char slave_addr, unsigned char reg_addr, unsigned char length, unsigned char *data){
	if(length==1){
		int ret = i2c_RegisterGet (I2C0, slave_addr*2, reg_addr, data);
		/*char buff[30];
		sprintf(buff,"\nRD1->ADDR: %02x, REG: %02x, DATA: %d",slave_addr,reg_addr,*data);
		uart_sendText(buff);
		sprintf(buff," <<%d>> ",ret);
		uart_sendText(buff);*/
		if(ret==1)	//TODO i2c_RegisterGet return 1 if success and this library requires 0 on success
			ret=0;
		return ret;
	}
	else {
		int ret = i2c_Register_Read_Block (I2C0, slave_addr*2, reg_addr, length, data);
		/*char buff[30];
		sprintf(buff,"\nREAD->ADDR: %02x, REG: %02x, LEN: %d DATA: ",slave_addr,reg_addr,length);
		uart_sendText(buff);
		for(int i=0;i<length;i++){
			sprintf(buff," [%d] ",data[i]);
			uart_sendText(buff);
